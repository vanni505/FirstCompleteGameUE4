...
AMyCharacter::AMyCharacter()
{
	...
	CameraBoom->bUsePawnControlRotation = true; //Ruota il braccio in base al controller. Il figlio segue la rotazione del genitore
	...
	FollowCamera->SetupAttachment(CameraBoom, USpringArmComponent::SocketName);//attacca le robe ai personaggi esempio spada. Lo SpringArmComponent ha un socket già integrato
    // Noi vgliamo che si muova lo springarm con la camera che resta ferma in una certa angolazione
	// Fissa la telecamera all'estremità del braccio e lasciare che il braccio si regoli in modo che corrisponda all'orientamento del controller
	FollowCamera->bUsePawnControlRotation = false; // gestisce i gli input che muovono un qualche cosa
	//Set our turn rates for input
	BaseTurnRate = 65.f; 
 	BaseLookUpRate = 65.f; // velocità con cui la telecamera si volta davanti e dietro
	// velocità di rotazione sia a destra che a sinistra quando premi "a" e "d" della telecamera
	// in un secondo può ruotare di 65 gradi

	// Don't rotate when the controller rotates.
	//Lascia che questo comando lo faccia solo la camera
...
void AMyCharacter::Tick(float DeltaTime)
{
	...
	case EStaminaStatus::ESS_Normal:
		if (bShiftKeyDown && (GetMainAnimInstance()->MovementSpeed)>0) 
		{
...
// Called to bind functionality to input
	...
	check(PlayerInputComponent); // controlla se è valido il player input component
	...
	PlayerInputComponent->BindAxis("MoveRight", this, &AMyCharacter::MoveRight); //prendi quella determinata funzione e passa l'input in ingresso
	
	PlayerInputComponent->BindAxis("Turn", this, &AMyCharacter::Turn); // andiamo ad ereditare la funzione già integrata nel pawn dal pawn stesso
...
{
	AddControllerYawInput(Rate * BaseTurnRate * GetWorld()->GetDeltaSeconds()); //Get World trasforma il delta time in secondi. Yaw è la rotazione rispetto all'asse Z.
}
...

{
	...
	{
		UKismetSystemLibrary::DrawDebugSphere(this, Location, 25.f, 8, FLinearColor::Black, 10.f, 0.5f);
	}
}
